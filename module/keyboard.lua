local keyboard = {}
local pressedChars, pressedCodes = {},{}
...
ks.exposeAPI("keyboard",keyboard)

--install a kernel process for the keyboard--

local kpid = ps.spawn([[
	if not ks then
		error("Keyboard module needs to run in kernelspace!")
	end
	local pressedChars, pressedCodes = ...
	while true do
		--busy loop~!--
		local event, address, char, code = computer.pullSignal()
...
]],"keyboard",{pressedChars, pressedCodes},true)
keyboard.pid = kpid
