...
ks.loadModule "text"
local keyboard = ks.getAPI "keyboard"
local text = ks.getAPI "text"

local term = {}
local cursorX, cursorY = 1, 1
local cursorBlink = {active=false,pid=nil,cursorX=1,cursorY=1}

cursorBlink.pid = ps.spawn([[
local cursorBlink, term = ...
while true do
	if cursorBlink.active and term.isAvailable() then
		cursorBlink.state = not cursorBlink.state
		local gpu = term.getGPU()
		if cursorBlink.state then
			local s, e = pcall(gpu.get,cursorBlink.cursorX, cursorBlink.cursorY)
			cursorBlink.alt = s and e or cursorBlink.alt
			gpu.set(cursorBlink.cursorX, cursorBlink.cursorY, string.rep(unicode.char(0x2588), unicode.charWidth(cursorBlink.alt))) -- solid block
		else
			gpu.set(cursorBlink.cursorX, cursorBlink.cursorY, cursorBlink.alt)
		end
		os.sleep(0.5)
	else
		os.sleep(1)
...
]],"term",{cursorBlink, term},true)

local gpu

function term.setGPU(g)
	gpu = g
end
...
function term.clear()
	...
	cursorX, cursorY = 1, 1
	cursorBlink.cursorX,cursorBlink.cursorY = cursorX,cursorY
end
...
function term.getCursorBlink()
	return cursorBlink.active
end

function term.setCursorBlink(enabled)
	checkArg(1, enabled, "boolean")
	cursorBlink.active = enabled
	cursorBlink.cursorX,cursorBlink.cursorY = cursorX,cursorY
	if enabled then
		if not cursorBlink.active then
...
function term.read(history, dobreak, hint, pwchar)
	...
	local function setCursor(nbx, nby)
		...
		if ncx > w then
			...
			scrollX = sx
			gpu.copy(1 + offset + dx, cy, w - offset - dx, 1, -dx, 0)
			local str = masktext(unicode.sub(history[nby], nbx - (dx - 1), nbx))
			str = text.padRight(str, dx)
			gpu.set(1 + math.max(offset, w - dx), cy, unicode.sub(str, 1 + math.max(0, dx - (w - offset))))
		elseif ncx < 1 + offset then
			local sx = nbx - 1
			local dx = math.abs(scrollX - sx)
			scrollX = sx
			gpu.copy(1 + offset, cy, w - offset - dx, 1, dx, 0)
			local str = masktext(unicode.sub(history[nby], nbx, nbx + dx))
			--str = text.padRight(str, dx)
			gpu.set(1 + offset, cy, str)
		end
	...
	local function insert(value)
		...
		local n = w - (cx - 1) - len
		if n > 0 then
			gpu.copy(cx, cy, n, 1, len, 0)
		end
		gpu.set(cx, cy, masktext(value))
