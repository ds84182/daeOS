--security module, manages secure things--

--hash api starts here--
...
do
	...
	hash.sha224 = hash224
	hash.sha256 = hash256
	hash.newSHA256 = new256
end

ks.exposeAPI("hash",hash)

--process elevation api starts here--

local sudo = {}
local secpid,secsigQ = ps.spawn(ks.getAPI("filesystem").read("/sys/bin/security_process.lua"),"security_process",nil,true)

sudo.pid = secpid
function sudo.spawnElevated(source, name, args, stdin, stdout, stderr, vars)
	local pid = ps.getCurrentProcess()
	local info = ps.getInfo(pid)
	if info.kernelspace then
		local npid, nsigQ = ps.spawn(source, name, args, true, stdin, stdout, stderr, vars)
		ps.pushProcessSignal("spawned_elevated",npid,nsigQ)
	else
		if config.security.password then
			secsigQ[#secsigQ+1] = {"spawn_elevated_password",pid,source,name,args,stdin,stdout,stderr,vars}
		else
			secsigQ[#secsigQ+1] = {"spawn_elevated",pid,source,name,args,stdin,stdout,stderr,vars}
		end
		ps.pause()
	end
end

ks.exposeAPI("sudo",sudo)
